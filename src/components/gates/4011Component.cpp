...
const std::unordered_map<std::size_t, std::pair<std::string, std::size_t>>
    nts::components::IC4011Component::INTERNAL_PIN_BINDS = {
...
nts::components::IC4011Component::IC4011Component()
    : AAdvancedComponent(PIN_NUMBER)
{
    for (int i = 1; i < 5; i++)
        this->_internalCircuit.addComponent(std::format("gate{}", i),
            Factory::createComponent("and"));
}

void nts::components::IC4011Component::simulate(const std::size_t tick)
{
...
nts::Tristate nts::components::IC4011Component::compute(const std::size_t pin)
{
    switch (pin)
    {
        case 3:
